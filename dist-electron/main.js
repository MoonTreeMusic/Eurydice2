...
function requireVideosize() {
  ...
  function createSizeFilters(output2, key, value) {
    var data2 = output2.sizeData = output2.sizeData || {};
    data2[key] = value;
    if (!("size" in data2)) {
      return [];
    }
    var fixedSize = data2.size.match(/([0-9]+)x([0-9]+)/);
    var fixedWidth = data2.size.match(/([0-9]+)x\?/);
    var fixedHeight = data2.size.match(/\?x([0-9]+)/);
    var percentRatio = data2.size.match(/\b([0-9]{1,3})%/);
    ...
    if (percentRatio) {
      ...
      aspect = width / height;
      if (data2.pad) {
        return getScalePadFilters(width, height, aspect, data2.pad);
      } else {
        return [{ filter: "scale", options: { w: width, h: height } }];
      }
    } else if (fixedWidth || fixedHeight) {
      if ("aspect" in data2) {
        width = fixedWidth ? fixedWidth[1] : Math.round(Number(fixedHeight[1]) * data2.aspect);
        height = fixedHeight ? fixedHeight[1] : Math.round(Number(fixedWidth[1]) / data2.aspect);
        width = Math.round(width / 2) * 2;
        height = Math.round(height / 2) * 2;
        if (data2.pad) {
          return getScalePadFilters(width, height, data2.aspect, data2.pad);
        } else {
          return [{ filter: "scale", options: { w: width, h: height } }];
        }
      } else {
      ...
      throw new Error("Invalid size specified: " + data2.size);
    }
  }
  ...
  (function(module) {
    (function() {
      ...
      async2.queue = function(worker, concurrency) {
        ...
        function _insert(q2, data2, pos, callback) {
          if (data2.constructor !== Array) {
            data2 = [data2];
          }
          _each(data2, function(task) {
            var item = {
              data: task,
        ...
        var q = {
          ...
          push: function(data2, callback) {
            _insert(q, data2, false, callback);
          },
          unshift: function(data2, callback) {
            _insert(q, data2, true, callback);
          },
          process: function() {
        ...
        var cargo = {
          ...
          process: function process2() {
            ...
            worker(ds, function() {
              working = false;
              var args = arguments;
              _each(ts, function(data2) {
                if (data2.callback) {
                  data2.callback.apply(null, args);
                }
              });
              process2();
            });
...
function requireProcessor() {
  ...
  function runFfprobe(command) {
    ...
    command.ffprobe(inputProbeIndex, function(err, data2) {
      command._ffprobeData = data2;
    });
  }
    ...
    proto._spawnFfmpeg = function(args, options, processCB, endCB) {
      ...
      this._getFfmpegPath(function(err, command) {
        ...
        if (options.captureStdout) {
          ffmpegProc.stdout.on("data", function(data2) {
            stdoutRing.append(data2);
          });
        ...
        ffmpegProc.stderr.on("data", function(data2) {
          stderrRing.append(data2);
        });
    ...
    proto._prepare = function(callback, readMetadata) {
      ...
      async2.waterfall([
        ...
        function(cb) {
          ...
          self2.ffprobe(0, function(err, data2) {
            if (!err) {
              self2._ffprobeData = data2;
            }
            cb();
          });
        },
  ...
  capabilities = function(proto) {
    ...
    proto.availableFilters = proto.getAvailableFilters = function(callback) {
      ...
      this._spawnFfmpeg(["-filters"], { captureStdout: true, stdoutLines: 0 }, function(err, stdoutRing) {
        ...
        callback(null, cache.filters = data2);
      });
    };
    proto.availableCodecs = proto.getAvailableCodecs = function(callback) {
      ...
      this._spawnFfmpeg(["-codecs"], { captureStdout: true, stdoutLines: 0 }, function(err, stdoutRing) {
          ...
          if (match && match[7] !== "=") {
            ...
            if (encoders.length || decoders.length) {
              ...
              encoders.forEach(function(name) {
                data2[name] = {};
                utils2.copy(coderData, data2[name]);
                data2[name].canEncode = true;
              });
              decoders.forEach(function(name) {
                if (name in data2) {
                  data2[name].canDecode = true;
                } else {
                  data2[name] = {};
                  utils2.copy(coderData, data2[name]);
                  data2[name].canDecode = true;
        ...
        callback(null, cache.codecs = data2);
      });
    };
    proto.availableEncoders = proto.getAvailableEncoders = function(callback) {
      ...
      this._spawnFfmpeg(["-encoders"], { captureStdout: true, stdoutLines: 0 }, function(err, stdoutRing) {
        ...
        callback(null, cache.encoders = data2);
      });
    };
        ...
        lines.forEach(function(line) {
          ...
          if (match) {
            match[3].split(",").forEach(function(format) {
              ...
              if (match[1] === "D") {
                data2[format].canDemux = true;
              }
              if (match[2] === "E") {
                data2[format].canMux = true;
              }
            });
          }
        });
        callback(null, cache.formats = data2);
      });
    };
...
function requireFfprobe() {
  ...
  function parseFfprobeOutput(out) {
    ...
    while (typeof line !== "undefined") {
      if (line.match(/^\[stream/i)) {
        ...
        data2.streams.push(stream);
      } else if (line.match(/^\[chapter/i)) {
        var chapter = parseBlock("chapter");
        data2.chapters.push(chapter);
      } else if (line.toLowerCase() === "[format]") {
        data2.format = parseBlock("format");
      }
      line = lines.shift();
    }
    return data2;
  ...
  ffprobe = function(proto) {
    proto.ffprobe = function() {
      ...
      var ended = false;
      function handleCallback(err, data2) {
        if (!ended) {
          ended = true;
          callback(err, data2);
        }
      ...
      this._getFfprobePath(function(err, path2) {
        ...
        function handleExit(err2) {
          ...
          if (processExited && stdoutClosed && stderrClosed) {
            ...
            var data2 = parseFfprobeOutput(stdout);
            [data2.format].concat(data2.streams).forEach(function(target) {
              if (target) {
        ...
        ffprobe2.stdout.on("data", function(data2) {
          stdout += data2;
        });
        ...
        ffprobe2.stderr.on("data", function(data2) {
          stderr += data2;
        });
...
function requireRecipes() {
  ...
  recipes = function recipes2(proto) {
    ...
    proto.mergeToFile = proto.concatenate = proto.concat = function(target, options) {
      ...
      var self2 = this;
      this.ffprobe(this._inputs.indexOf(fileInput), function(err, data2) {
        if (err) {
          return self2.emit("error", err);
        }
        var hasAudioStreams = data2.streams.some(function(stream) {
          return stream.codec_type === "audio";
        });
        var hasVideoStreams = data2.streams.some(function(stream) {
...
function getLibraryPath() {
  return path$2.join(electron.app.getPath("userData"), "library.json");
}
let data = { tracks: [], settings: {}, nextId: 1, playlists: [], nextPlaylistId: 1 };
function loadLibrary() {
  try {
    const raw = require$$0.readFileSync(getLibraryPath(), "utf-8");
    data = JSON.parse(raw);
...
function saveLibrary() {
  require$$0.writeFileSync(getLibraryPath(), JSON.stringify(data), "utf-8");
}
...
async function collectAudioFiles(dir, results) {
  try {
    const entries = require$$0.readdirSync(dir, { withFileTypes: true });
    for (const entry of entries) {
      const fullPath = path$2.join(dir, entry.name);
      if (entry.isDirectory()) {
        await collectAudioFiles(fullPath, results);
      } else if (AUDIO_EXTS.has(path$2.extname(entry.name).toLowerCase())) {
...
const __dirname$1 = path$2.dirname(url.fileURLToPath(typeof document === "undefined" ? require("url").pathToFileURL(__filename).href : _documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === "SCRIPT" && _documentCurrentScript.src || new URL("main.js", document.baseURI).href));
...
function createWindow() {
  ...
  if (isDev) {
    ...
    mainWindow.loadFile(path$2.join(__dirname$1, "../dist/index.html"));
  }
}
